...
const XFilterTable = defineComponent({
  props: {
    data: {
      type: Object as PropType<SortTableData>,
      default: () => ({
        columns: [],
        rows: [],
      }),
  ...
  setup(props) {
    ...
    const filteredRows = ref(JSON.parse(JSON.stringify(props.data.rows)) as string[][])
    const columnFilters = ref<Record<string, ColumnFilter>>({})

    function updateFilteredAndSortedData() {
      let result: SortTableData['rows'] = JSON.parse(JSON.stringify(props.data.rows))
    ...
    // 处理排序
    function handleSort(columnIndex: number, order: 'asc' | 'desc') {
